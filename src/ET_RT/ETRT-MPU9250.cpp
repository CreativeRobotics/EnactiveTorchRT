...
{
	//Most of this is not needed - just for display purposes - move to new function and just have a simple init
...
//Add pointers to raw data arrays for storage of bias values in flash
...
//Apply gyro and accelerometer offsets gathered during calibration (or recalled from FLASH)
void  MPU9250::applyOffsets(){ //we assume that the correct bias values have been assigned
	//2% flash
	uint8_t data[6];
...
// Push accelerometer biases to hardware registers
  ...
  writeByte(MPU9250_ADDRESS, ZA_OFFSET_L, data[5]);
	...
	// Output scaled biases for display in the main program
	for(int n = 0; n < 3; n++){
		gyroBias[n] = (float) gyro_bias[n]/(float) gyrosensitivity; 
		accelBias[n] = (float)accel_bias[n]/(float)accelsensitivity;		
	}
